...
resume: |
    La diffusion et l'exploitation de microdonnées se heurte souvent à des freins techniques liés au volume des fichiers et à la complexité de leur exploitation. Le format Parquet, issu à l'origine de l’écosystème big data mais s'étant diffusé bien au-delà de cette sphère, s'impose comme une alternative efficace aux formats traditionnels comme le CSV. Cette présentation reviendra sur les expériences de diffusion de données dans ce format. Ce choix technique, associé à un accompagnement des utilisateurs (guides, exemples de code), a permis d'accroître l'usage de ces sources qui étaient, auparavant, complexes à valoriser.  Cette communication évoquera également les implications pratiques de ce choix technique, les bonnes pratiques à mettre en oeuvre lors de l'adoption de ce format de données et les opportunités d'innovation permises par ce format.
abstract: |
    The dissemination and use of microdata often face technical obstacles related to the size of the files and the complexity of working with them. The Parquet format, originally developed within the big data ecosystem, is now widely adopted. It has emerged as an efficient alternative to traditional formats such as CSV. This presentation will discuss experiences in distributing data in this format. The technical choice of Parquet, combined with user support (guides, code examples), has helped increase the use of these data sources, which were previously difficult to exploit. The talk will also address the practical implications of this technical choice, best practices for adopting this data format, and the opportunities for innovation that it enables.
...
Parmi les nombreuses innovations techniques ayant émergé au cours des années 2010 dans l'écosystème de la donnée, le format `Parquet` fait parti de celles ayant connues une adoption large parmi les _data scientists_ et statisticiens. Initialement conçu pour répondre aux besoins des plateformes _big data_ comme [Hadoop](https://fr.wikipedia.org/wiki/Hadoop) - réduction du volume de stockage et du temps de lecture des données - celui-ci s'est diffusé au-delà de cette sphère et a même survécu à la plupart des technologies _big data_ qui sont, depuis, tombées en désuétude [@Tigani_2023].
...
![Représentation sur le *filesystem* d'un fichier `Parquet` partitionné selon deux clés : l'année et le mois (empruntée à @mise_en_prod)](https://ensae-reproductibilite.github.io/website/parquet-partition.png){#fig-parquet-partitions fig-align="center" height=300}
...
![Représentation en mémoire des données au format `Arrow` (empruntée à la [documentation du projet Arrow](https://arrow.apache.org/overview/))](https://ensae-reproductibilite.github.io/website/arrow-memory.png){#fig-arrow-memory fig-align="center" height=400}
...
![_DuckDB_, un outil universel pour lire des données (Source: [MotherDuck](https://motherduck.com/blog/duckdb-enterprise-5-key-categories/))](https://ensae-reproductibilite.github.io/website/duckdb-multisrc.png){#fig-duckdb-multisrc fig-align="center" height=400}
...
Outre une utilisation efficace des ressources, la diffusion dans ce format apporte aussi du confort aux utilisateurs, notamment en montrant comment lire ces données avec `DuckDB`. Comme cette librairie est simple d'usage, le public cible de la diffusion de ces données, majoritairement utilisateur de `R` ou `Python`, pouvait ainsi s'approprier rapidement ce nouveau format. Pour illustrer cette simplicité d'usage, prenons pour exemple un besoin précis: dénombrer les logements niçois dont les occupants ont emménagé depuis 2020. En `Python`, cela se traduit par la requête SQL:
...
#titled-raw-block(filename: "Un exemple d'utilisation des données diffusées par l'Insee avec DuckDB et R", 
  raw(lang: "Python", "library(duckdb)
